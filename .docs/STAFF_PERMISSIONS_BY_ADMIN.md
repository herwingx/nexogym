...
El admin del gym puede **activar o desactivar módulos por persona** para el personal (Recepción, Coach, Instructor). Así una misma persona puede tener una sola cuenta y hacer varias funciones: por ejemplo un coach que también vende en mostrador, o un recepcionista que también edita rutinas.
...
No hace falta crear dos cuentas si la misma persona hace recepción y rutinas: se crea con un rol y el admin activa el otro módulo en **Permisos**.
...
2. En la fila del usuario (solo Recepcionista, Coach o Instructor), clic en **Permisos**.
...
- **Campo en BD:** `User.staff_permissions` (JSON opcional). Claves: `can_use_pos`, `can_use_routines`, `can_use_reception`, `can_view_dashboard`, `can_view_members_admin`, `can_use_finance`, `can_manage_staff`, `can_view_audit`, `can_use_gamification`.
...
- **Permisos adicionales** permiten dar acceso a Dashboard, Socios admin, Finanzas, Personal, Auditoría o Gamificación a staff de confianza.
